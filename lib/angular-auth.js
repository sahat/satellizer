...
(function(window, angular, undefined) {
  ...
  angular.module('ngAuth', []).provider('$auth', function() {
    ...
    this.setProvider = function(params) {
      if (!params.name) throw new Error('You did not set a provider\'s name.');
      if (!config.providers[params.name]) throw new Error('Provider ' + params.name + ' is not found. Are you trying to add a custom provider? Use addProvider() instead.');
      angular.extend(config.providers[params.name], params);
    };

    this.addProvider = function(params) {
      if (!params.name) throw new Error('You did not set a provider\'s name.');
      if (config.providers[params.name]) throw new Error('Provider ' + params.name + ' already exists. Use setProvider() instead.');
      config.providers[params.name] = {};
      angular.extend(config.providers[params.name], params);
    };
