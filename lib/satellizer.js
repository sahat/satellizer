...
(function(window, angular, undefined) {
  ...
  function $auth() {
    ...
    this.$get = function(OAuth1, OAuth2, $http, $location, $rootScope, $q, $window) {
      ...
      $auth.authenticate = function(providerName) {
        ...
        var provider = (providers[providerName].type === 'oauth1') ?
          (new OAuth1(providers[providerName])) :
          (new OAuth2(providers[providerName]));
  ...
  function onRun($rootScope, $window, $location, parseQueryString) {
  ...
  function utils() {
    this.camelCase = function camelCase(name) {
      return name.replace(/([\:\-\_]+(.))/g, function(_, separator, letter, offset) {
        return offset ? letter.toUpperCase() : letter;
      });
    };

    this.parseQueryString = function parseQueryString(keyValue) {
      var obj = { }, key, value;
