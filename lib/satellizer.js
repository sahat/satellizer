...
(function(window, angular, undefined) {
  ...
  function $auth() {
    ...
    this.$get = function(Oauth1, Oauth2, Local, $http, $q) {
      ...
      $auth.authenticate = function(providerName) {
        ...
        provider.open(providers[providerName]).then(function(response) {
  ...
  function RunBlock($rootScope, $window, $location, Utils) {
    return {
      run: function() {
        ...
        if (token) {
          var payload = JSON.parse($window.atob(token.token.split('.')[1]));
